...
void proprioception_callback(const cassie_common_toolbox::cassie_proprioception_msg::ConstPtr& propmsg)
{
    ...
    is_initialized = propmsg->isSimRunning;
}
...
int main(int argc, char* argv[])
{
    ...
    int qpulog_size, qplog_size;

    switch (control_mode) {
    case 1:            //walking
        qpulog_size = (isRigid) ? 74 : 70;
        qplog_size = 83 + 2; //HLIP stepping desired speed
        break;
    case 0:            //standing
        qpulog_size = (isRigid) ? 76 : 72;
        qplog_size = 68;
        break;
    case -1: //in air
        qpulog_size = 76;
    ...
    StandingControlTSC stand_control(nh, robot); //kept intact
    ...
    if (log_controller)
    {
        ...
        path = home + "/datalog/qp_u_walk_log.bin";
        logfileWalku.open(path, std::ios::out | std::ios::binary);

        path = home + "/datalog/stand_log.bin";
